...
const DevTicketScreen = () => {
  ...
  const getColorClasses = (color: any) => {
    const colors = {
      success: {
        bg: 'bg-[#127749]',
        bgLight:'bg-[rgba(42,158,106,0.1)]',
        text: 'text-[#2A9E6A]',
        border: 'border-[rgba(18,119,73,0.4)]'
      },
      warning: {
        bg: 'bg-[#E3B341]',
        bgLight:'bg-[rgba(227,179,65,0.1)]',
        text: 'text-[#E3B341]',
  ...
  return (
    <div className="space-y-3 md:space-y-6">
      {/* Header */}
      <div className={`rounded-2xl shadow-sm border p-3 md:p-6 ${'bg-white border-gray-300'}`}>
        <div className="flex flex-wrap items-center gap-3 md:gap-4 mb-4">
          <div className={`p-2 md:p-3 rounded-xl ${'bg-[rgba(160,120,75,0.12)]'}`}>
            <FileText size={isMobile ? 20 : 28} className={'text-[#6B4D30]'} />
          </div>
          <div>
            <h1 className={`text-lg md:text-2xl font-bold font-['Montserrat'] ${'text-gray-800'}`}>
              {t('devTicket.title')}
            </h1>
        ...
        {/* Flow Diagram */}
        <div className={`rounded-xl border p-4 ${'bg-gray-50 border-gray-300'}`}>
          <div className="flex items-center gap-2 mb-3">
            <GitBranch size={16} className={'text-gray-700'} />
            <span className={`text-sm font-semibold font-['Montserrat'] ${'text-gray-700'}`}>
              ...
              return (
                <React.Fragment key={page.id}>
                  ...
                  {idx < PLANNING_PAGES.length - 1 && (
                    <ArrowRight size={20} className={`${'text-gray-700'} ${isMobile ? 'rotate-90' : ''}`} />
                  )}
                </React.Fragment>
              );
            })}
      ...
      <div className="space-y-4">
        {PLANNING_PAGES.map((page) => {
          ...
          return (
            <div
              key={page.id}
              className={`rounded-xl border overflow-hidden ${'bg-white border-gray-300'}`}
            >
              {/* Page Header */}
              <button
                type="button"
                onClick={() => togglePage(page.id)}
                className={`w-full flex items-center gap-2 md:gap-4 px-3 md:px-5 py-3 md:py-4 ${colors.bgLight} border-b transition-colors ${'border-gray-300 hover:bg-[rgba(160,120,75,0.18)]'}`}
              >
                <ChevronRight
                  size={isMobile ? 16 : 20}
                  className={`flex-shrink-0 transition-transform ${'text-gray-700'} ${isExpanded ? 'rotate-90' : ''}`}
                />
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                    <h2 className={`text-sm md:text-lg font-semibold font-['Montserrat'] ${'text-gray-800'}`}>
                      {page.name}
                    </h2>
              ...
              {isExpanded && (
                <div className="p-3 md:p-5 space-y-3 md:space-y-4">
                  {/* Basic Info */}
                  <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 p-3 md:p-4 rounded-lg ${'bg-gray-50'}`}>
                    <div>
                      <span className={`text-xs font-medium ${'text-gray-700'}`}>{t('devTicket.component')}</span>
                      <p className={`text-sm font-['JetBrains_Mono'] mt-1 ${'text-gray-700'}`}>{page.component}</p>
                    </div>
                    <div>
                      <span className={`text-xs font-medium ${'text-gray-700'}`}>{t('devTicket.filePath')}</span>
                      <p className={`text-sm font-['JetBrains_Mono'] mt-1 ${'text-gray-700'}`}>{page.file}</p>
                    </div>
                    <div>
                      <span className={`text-xs font-medium ${'text-gray-700'}`}>{t('devTicket.route')}</span>
                      <p className={`text-sm font-['JetBrains_Mono'] mt-1 ${'text-gray-700'}`}>{page.route}</p>
                    </div>
                    <div>
                      <span className={`text-xs font-medium ${'text-gray-700'}`}>{t('devTicket.screenId')}</span>
                      <p className={`text-sm font-['JetBrains_Mono'] mt-1 ${'text-gray-700'}`}>{page.id}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    <button
                      ...
                      className={`flex items-center gap-2 mb-3 transition-colors ${'text-gray-700 hover:text-[#6B4D30]'}`}
                    >
                      <ChevronDown
                        size={16}
                        ...
                        <div className="space-y-2">
                          {page.props.map((prop) => (
                            <MobileDataCard
                              ...
                              metrics={[
                                { label: 'Type', value: prop.type, color:undefined} as any,
                              ]}
                            />
                          ))}
                        </div>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className={'bg-[rgba(160,120,75,0.18)]'}>
                                <th className={`px-4 py-2 text-left text-xs font-semibold font-['Montserrat'] ${'text-[#666666]'}`}>{t('common.name')}</th>
                                <th className={`px-4 py-2 text-left text-xs font-semibold font-['Montserrat'] ${'text-[#666666]'}`}>Type</th>
                                <th className={`px-4 py-2 text-left text-xs font-semibold font-['Montserrat'] ${'text-[#666666]'}`}>{t('common.description')}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {page.props.map((prop, idx) => (
                                <tr key={prop.name} className={`${idx % 2 === 0 ? ('bg-white') : ('bg-gray-50')}`}>
                                  <td className={`px-4 py-2 font-['JetBrains_Mono'] text-xs ${'text-[#6B4D30]'}`}>{prop.name}</td>
                                  <td className={`px-4 py-2 font-['JetBrains_Mono'] text-xs ${'text-amber-600'}`}>{prop.type}</td>
                    ...
                    {expandedSections[`${page.id}_navigation`] && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                        <div className={`p-3 rounded-lg ${'bg-gray-50'}`}>
                          <span className={`text-xs font-medium ${'text-gray-700'}`}>{t('devTicket.navigateFrom')}</span>
                          <div className="mt-2 space-y-1">
                            {page.navigationFrom.length > 0 ? (
                              page.navigationFrom.map(nav => {
                                const navPage = PLANNING_PAGES.find(p => p.id === nav);
                                return navPage ? (
                                  <div key={nav} className={`flex items-center gap-2 text-sm ${'text-gray-700'}`}>
                              ...
                              <span className={`text-sm ${'text-gray-700'}`}>{t('common.entryPointSidebar')}</span>
                            )}
                          </div>
                        </div>
                        <div className={`p-3 rounded-lg ${'bg-gray-50'}`}>
                          <span className={`text-xs font-medium ${'text-gray-700'}`}>{t('devTicket.navigateTo')}</span>
                          <div className="mt-2 space-y-1">
                            {page.navigationTo.length > 0 ? (
                              ...
                              <span className={`text-sm ${'text-gray-700'}`}>{t('common.endOfFlow')}</span>
                            )}
                          </div>
                        </div>
                      </div>
      ...
      {/* Summary Stats */}
      <div className={`rounded-xl border p-3 md:p-5 ${'bg-white border-gray-300'}`}>
        <div className="flex items-center gap-2 mb-4">
          <Database size={16} className={'text-gray-700'} />
          <span className={`text-sm font-semibold font-['Montserrat'] ${'text-gray-700'}`}>{t('devTicket.summary')}</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {/* Total Pages */}
          <div
            className={`relative overflow-hidden border rounded-2xl p-4 text-center transition-all duration-200 hover:shadow-lg group ${'border-gray-300'}`}
            style={{
              background:`linear-gradient(135deg, #ffffff 0%, #ffffff 55%, ${CARD_ACCENTS.pages.lightGrad} 100%)`}}
          >
            <div
            ...
            <div
              ...
              style={{ backgroundColor:CARD_ACCENTS.pages.iconLight}}
            >
              <FileText size={14} color={CARD_ACCENTS.pages.color} />
            </div>
            <div className="relative z-10">
              <p className={`text-2xl font-bold font-['JetBrains_Mono'] ${'text-gray-800'}`}>{PLANNING_PAGES.length}</p>
              <p className={`text-xs mt-1 ${'text-gray-700'}`}>{t('devTicket.totalPages')}</p>
            </div>
          </div>
          ...
          <div
            className={`relative overflow-hidden border rounded-2xl p-4 text-center transition-all duration-200 hover:shadow-lg group ${'border-gray-300'}`}
            style={{
              background:`linear-gradient(135deg, #ffffff 0%, #ffffff 55%, ${CARD_ACCENTS.done.lightGrad} 100%)`}}
          >
            <div
              className="absolute -bottom-2 -right-2 pointer-events-none"
              style={{ opacity:0.06}}
            ...
            <div
              className="absolute top-3 right-3 w-8 h-8 rounded-lg flex items-center justify-center backdrop-blur-sm"
              style={{ backgroundColor:CARD_ACCENTS.done.iconLight}}
            >
            ...
            <div className="relative z-10">
              ...
              <p className={`text-xs mt-1 ${'text-gray-700'}`}>{t('devTicket.completed')}</p>
            </div>
          </div>
          ...
          <div
            className={`relative overflow-hidden border rounded-2xl p-4 text-center transition-all duration-200 hover:shadow-lg group ${'border-gray-300'}`}
            style={{
              background:`linear-gradient(135deg, #ffffff 0%, #ffffff 55%, ${CARD_ACCENTS.props.lightGrad} 100%)`}}
          >
            <div
              className="absolute -bottom-2 -right-2 pointer-events-none"
              style={{ opacity:0.06}}
            ...
            <div
              className="absolute top-3 right-3 w-8 h-8 rounded-lg flex items-center justify-center backdrop-blur-sm"
              style={{ backgroundColor:CARD_ACCENTS.props.iconLight}}
            >
              <Code2 size={14} color={CARD_ACCENTS.props.color} />
            </div>
            <div className="relative z-10">
              <p className={`text-2xl font-bold font-['JetBrains_Mono'] ${'text-gray-800'}`}>
                {PLANNING_PAGES.reduce((sum, p) => sum + p.props.length, 0)}
              </p>
              <p className={`text-xs mt-1 ${'text-gray-700'}`}>{t('devTicket.totalProps')}</p>
            </div>
          </div>
          ...
          <div
            className={`relative overflow-hidden border rounded-2xl p-4 text-center transition-all duration-200 hover:shadow-lg group ${'border-gray-300'}`}
            style={{
              background:`linear-gradient(135deg, #ffffff 0%, #ffffff 55%, ${CARD_ACCENTS.features.lightGrad} 100%)`}}
          >
            <div
              className="absolute -bottom-2 -right-2 pointer-events-none"
              style={{ opacity:0.06}}
            ...
            <div
              className="absolute top-3 right-3 w-8 h-8 rounded-lg flex items-center justify-center backdrop-blur-sm"
              style={{ backgroundColor:CARD_ACCENTS.features.iconLight}}
            >
            ...
            <div className="relative z-10">
              ...
              <p className={`text-xs mt-1 ${'text-gray-700'}`}>{t('devTicket.totalFeatures')}</p>
            </div>
          </div>
        </div>
      </div>
