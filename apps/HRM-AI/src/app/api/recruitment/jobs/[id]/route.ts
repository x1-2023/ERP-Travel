...
import { getJobPostingById, updateJobPosting } from '@/services/recruitment/job-posting.service'
...
export async function GET(
  ...
  try {
    ...
    const result = await getJobPostingById(id, session.user.tenantId)
...
export async function PUT(
  ...
  try {
    ...
    const result = await updateJobPosting(id, ctx.tenantId, body, ctx)
    return NextResponse.json({ success: true, data: result })
  } catch (error) {
