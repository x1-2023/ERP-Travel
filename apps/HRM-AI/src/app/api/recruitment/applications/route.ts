...
import { getApplications, createApplication } from '@/services/recruitment/application.service'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await getApplications(tenantId, filters, page, pageSize)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createApplication(ctx.tenantId, body)
