...
- 2026-06-06: Checked local `Waitress Rig` folder. It contains a Maya 2020 `.ma` rig with facial/lip controls, but Blender cannot open it and no Maya exporter is available in PATH. Added a Three.js procedural 3D mouth prototype as the default mouth panel with a 2D fallback toggle, using the existing viseme stream.
